...
exports.registerTemporalTools = registerTemporalTools;
const time_math_1 = require("../utils/time-math");
const TASK_CATEGORIES = ['research', 'coding', 'writing', 'analysis', 'communication', 'scheduling', 'file-operations', 'web-browsing', 'other'];
const TASK_COMPLEXITIES = ['trivial', 'simple', 'moderate', 'complex', 'highly-complex'];
function registerTemporalTools(ctx) {
    const tools = [];
    // temporal_now
    tools.push({
        name: 'temporal_now',
        ...
        execute: async () => {
            if (!ctx.timePerception)
                return { error: 'Time perception not available' };
    ...
    tools.push({
        ...
        execute: async ({ category, complexity }) => {
            if (!ctx.taskEstimator)
                return { error: 'Task estimator not available' };
    ...
    tools.push({
        ...
        execute: async (params) => {
            if (!ctx.priorityScheduler || !ctx.taskEstimator)
                return { error: 'Scheduler not available' };
            const estimatedDuration = ctx.taskEstimator.estimate(params.category, params.complexity);
            let deadlineMs;
            if (params.deadline) {
                const relativeMs = (0, time_math_1.parseRelativeTime)(params.deadline);
    ...
    tools.push({
        ...
        execute: async () => {
            if (!ctx.cycleManager)
                return { error: 'Cycle manager not available' };
    ...
    tools.push({
        ...
        execute: async ({ durationMs }) => {
            ...
            const estimate = { minimumMs: durationMs * 0.8, expectedMs: durationMs, maximumMs: durationMs * 1.5, confidence: 0.7, basedOnSamples: 0, category: 'other', complexity: 'moderate' };
    ...
    tools.push({
        ...
        parameters: { type: 'object', properties: {}, required: [] },
        execute: async () => {
            if (!ctx.crossChannelSync)
                return { error: 'Cross-channel sync not available' };
    ...
    tools.push({
        ...
        execute: async ({ query, maxAgeDays, limit }) => {
            if (!ctx.temporalMemory)
...
//# sourceMappingURL=temporal-tools.js.map